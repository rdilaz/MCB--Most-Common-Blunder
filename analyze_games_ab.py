...
def check_for_missed_mate(board_before, best_move_info, after_move_eval, turn_color, move_num):
    ...
    if best_move_eval.is_mate() and best_move_eval.mate() > 0 and not after_move_eval.is_mate():
        ...
        description = f"You missed a checkmate in {mate_in} moves. The best move was {best_move_san}."
...
def check_for_allowed_mate(board_before, after_move_eval, move_num, move_played, best_move_info, info_after_move):
    if after_move_eval.is_mate() and after_move_eval.mate() < 0:
        ...
        description = f"Your move {move_played_san} allows the opponent to force checkmate in {abs(after_move_eval.mate())} moves."
...
def check_for_missed_fork(board_before, best_move_info, turn_color):
    ...
    if len(valuable_pieces_attacked) >= 2:
        ...
        return {"category": "Missed Fork", "move_number": move_num, "description": description, "punishing_move": best_move, "specificity": 5}
    return None
...
def allowed_fork(board_after, info_after_move, turn_color):
    ...
    if opponent_best_move not in board_after_opponent_move.legal_moves: return None
    
    opponent_best_move_san = board_after.san(opponent_best_move)
    # print(f"[DEBUG][allowed_fork] Engine's best move for opponent: {opponent_best_move_san}")
    ...
    if len(valuable_pieces_attacked) >= 2:
        # print(f"[DEBUG][allowed_fork] Fork detected! Attacked pieces: {[PIECE_NAMES.get(p['piece'].piece_type) for p in valuable_pieces_attacked]}")
...
def categorize_blunder(board_before, board_after, move_played, info_before_move, info_after_move, best_move_info, blunder_threshold, engine, engine_think_time):
    move_played_san = board_before.san(move_played)
    ...
    check_functions = [
        ...
        (allowed_fork, [board_after, info_after_move, turn_color]),
    ...
    print("[DEBUG] Step 1: Collecting all possible blunder categories...")
    for func, args in check_functions:
        result = func(*args)
        if result:
            print(f"[DEBUG]   - Found possible blunder: {result['category']}")
            possible_blunders.append(result)

    print(f"[DEBUG] Collected {len(possible_blunders)} possible blunder(s): {[b['category'] for b in possible_blunders]}")

    if not possible_blunders:
        print("[DEBUG] No specific tactical blunders found. Checking for win probability drop...")
        ...
        if win_prob_drop >= blunder_threshold:
            print("[DEBUG]   - Drop is above threshold. Categorizing as 'Mistake'.")
        ...
        print("[DEBUG]   - Drop is below threshold. Not a blunder.")
        return None
    ...
    for blunder in possible_blunders:
        ...
        if punishing_move:
             # Need the correct board state to get the SAN for the punishing move
            board_for_san = board_after if blunder['category'].startswith("Allowed") else board_before
            punishing_move_san = board_for_san.san(punishing_move)

        print(f"[DEBUG]   Evaluating: {blunder['category']}")
        print(f"[DEBUG]     - Punishing Move: {punishing_move_san}")
        if punishing_move:
            temp_board = board_after.copy()
            if punishing_move in temp_board.legal_moves:
                print(f"[DEBUG]     - Making extra engine call for {punishing_move_san}...")
                ...
                print(f"[DEBUG]     - Calculated Severity Score: {blunder['severity_score']}")
            else:
                print(f"[DEBUG]     - Punishing move {punishing_move_san} is not legal. Discarding.")
                blunder["severity_score"] = 99999 
        else:
            blunder["severity_score"] = blunder.get("severity_score", 0)
            print(f"[DEBUG]     - Using pre-calculated Severity Score: {blunder['severity_score']}")
    ...
    final_blunder = min(ranked_blunders, key=lambda x: (x["severity_score"], -x.get("specificity", 0)))
    print(f"\n[DEBUG] --- Final Decision ---")
    print(f"[DEBUG] Selected Blunder: {final_blunder['category']} (Score: {final_blunder['severity_score']})")
...
def analyze_game(game, engine, target_user, blunder_threshold, engine_think_time):
    ...
    for move in game.mainline_moves():
        if board.turn == user_color:
            ...
            blunder_info = categorize_blunder(board_before, board, move, info_before_move, info_after_move, best_move_info, blunder_threshold, engine, engine_think_time)
            if blunder_info:
